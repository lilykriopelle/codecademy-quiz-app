...
In **Topics.js**, import the selector defined in your slice and use it to access all the topics in state. We've already written the code that iterates over the topics in state, putting a `Link` on the page for each one. 

Likewise, in **Topic.js**, import the selector defined in your slice and use it to select the topics in state. We've written the code that pulls the topic's `id` out of the route params and uses it to select the correct topic from all the topics in state.
